...
function checkCeiling(cords){
		for (var i =0;i<plats.length;i++){
		p = plats[i];
		if (p.x<cords[0]+10*contSize&&p.x+p.width>cords[0]+3&&p.y<cords[1]&&p.y+p.height>cords[1]){
			return false;
		}
	}
	return true;	
...
function checkSides(cords){
		...
		p = plats[i];
		fill(255,0,0);
		rect(cords[0],cords[1],10,10);
		if (p.x<cords[0]&&
			p.x+p.width>cords[0]&&
			p.y<cords[1]+18*contSize&&
			p.y+p.height>cords[1]){
			return false;
		}else if(p.x+contSize*2<cords[0]+13*contSize&&
				 p.x+p.width-contSize*2>cords[0]+13*contSize&&
				 p.y<cords[1]-3+contSize*18&&
...
function platform(x,y,width,height){
	...
	this.draw=function(){
		fill(140);
		image(midplat, this.x,this.y,this.width,this.height);
	}
